...
test('Inicio surfaces persistent Decision Cases without creating another module', () => {
  assert.match(shell, /pathname === '\/dashboard' \? <HomeDecisionPriorities \/>/);
  assert.match(priorities, /\/api\/intelligence\/decision-cases\?status=open/);
  assert.match(priorities, /slice\(0, 3\)/);
});
