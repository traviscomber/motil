...
function primaryReason(item: DecisionCase) {
  const factors = item.attention?.factors || [];
  return factors
...
export function HomeDecisionPriorities() {
  const state = useSWR<Payload>('/api/intelligence/decision-cases/prioritized?limit=3', fetcher, {
            ...
            <Card key={item.id} className="shadow-none">
              <CardContent className="space-y-3 p-4">
                <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    {item.attention?.level ? <Badge variant="outline">{item.attention.level}</Badge> : null}
